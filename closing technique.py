...
def apply_closing(image_path):
    ...
    # A larger kernel will close larger gaps
    kernel = np.ones((5, 5), np.uint8)  # 5x5 kernel

    # Apply the Closing operation
    closing = cv2.morphologyEx(image, cv2.MORPH_CLOSE, kernel)

    # Display the original and Closing images
    cv2.imshow("Original Image", image)
    cv2.imshow("Closing Image", closing)
    cv2.waitKey(0)  # Wait for a key press to close the windows
    cv2.destroyAllWindows()  # Close all OpenCV windows

    # Save the Closing image (optional)
    output_path = "closing_image.jpg"
    cv2.imwrite(output_path, closing)
    print(f"Closing image saved as {output_path}")
